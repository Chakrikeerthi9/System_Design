...
interface Stats {
  fixed_window:   { count: number; limit: number; remaining: number };
  sliding_window: { count: number; limit: number; remaining: number };
  token_bucket:   { tokens: number; capacity: number };
}
  ...
  return (
    <div className="gauge-wrap">
      <svg viewBox="0 0 140 140" width="140" height="140">
        ...
        <text x="70" y="82" textAnchor="middle" fill="#888" fontSize="9"
          fontFamily="'JetBrains Mono', monospace">/ {max}</text>
      </svg>
...
export default function Dashboard() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/rate-limiter/stats`);
  ...
  const fireRequest = useCallback(async () => {
    ...
    setFiring(true);
    try {
      const res = await fetch(`${API_BASE}/rate-limiter${activeStrategy.endpoint}`, { method: "POST" });
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: #080810; color: #e0e0e0; font-family: 'JetBrains Mono', monospace; min-height: 100vh; overflow-x: hidden; }
        .bg-grid { position: fixed; inset: 0; z-index: 0; background-image: linear-gradient(rgba(0,255,136,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(0,255,136,0.03) 1px, transparent 1px); background-size: 40px 40px; pointer-events: none; }
        .container { position: relative; z-index: 1; max-width: 1200px; margin: 0 auto; padding: 32px 24px; }
        .header { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 40px; border-bottom: 1px solid #1e1e3a; padding-bottom: 24px; }
        .header-badge { display: inline-block; font-size: 10px; letter-spacing: 0.2em; color: #00ff88; border: 1px solid #00ff8840; padding: 4px 10px; border-radius: 2px; margin-bottom: 10px; text-transform: uppercase; }
        .header-title { font-family: 'Syne', sans-serif; font-size: 28px; font-weight: 800; color: white; letter-spacing: -0.02em; line-height: 1.1; }
        .header-title span { color: #00ff88; }
        .header-sub { font-size: 11px; color: #555; margin-top: 6px; }
        .header-metrics { display: flex; gap: 24px; align-items: center; }
        .metric-pill { text-align: right; }
        ...
        @media (max-width: 768px) { .grid-top { grid-template-columns: 1fr; } .header { flex-direction: column; gap: 16px; } }
        .card { background: #0d0d1a; border: 1px solid #1e1e3a; border-radius: 8px; padding: 20px; }
        .card-title { font-size: 9px; letter-spacing: 0.2em; color: #555; text-transform: uppercase; margin-bottom: 16px; }
        .strategy-tabs { display: flex; gap: 8px; margin-bottom: 16px; flex-wrap: wrap; }
        .strategy-tab { padding: 8px 14px; border-radius: 4px; border: 1px solid #1e1e3a; background: transparent; cursor: pointer; font-family: 'JetBrains Mono', monospace; font-size: 11px; color: #666; transition: all 0.2s; }
        .strategy-tab:hover { border-color: #333; color: #999; }
        .strategy-tab.active { background: #111125; color: white; }
        .strategy-info { padding: 14px; background: #060610; border-radius: 6px; margin-bottom: 16px; border-left: 3px solid var(--sc); }
        ...
        .btn { flex: 1; padding: 12px; border: none; border-radius: 4px; font-family: 'JetBrains Mono', monospace; font-size: 12px; font-weight: 600; cursor: pointer; transition: all 0.15s; letter-spacing: 0.05em; }
        .btn-fire { background: #00ff88; color: #080810; }
        ...
        .btn-burst { background: transparent; color: #ff6b35; border: 1px solid #ff6b3540; }
        .btn-burst:hover { background: #ff6b3510; border-color: #ff6b35; }
        .btn-burst:disabled { opacity: 0.3; cursor: not-allowed; }
        .gauges-row { display: flex; justify-content: space-around; align-items: center; gap: 8px; }
        ...
        .log-clear { font-size: 9px; color: #333; cursor: pointer; background: none; border: none; font-family: 'JetBrains Mono', monospace; letter-spacing: 0.1em; }
        .log-clear:hover { color: #666; }
        .log-body { height: 240px; overflow-y: auto; padding: 8px 0; }
        .log-body::-webkit-scrollbar { width: 4px; }
        .log-body::-webkit-scrollbar-track { background: transparent; }
        .log-body::-webkit-scrollbar-thumb { background: #1e1e3a; border-radius: 2px; }
        .log-entry { display: flex; gap: 10px; align-items: center; padding: 5px 16px; font-size: 11px; border-left: 2px solid transparent; transition: background 0.1s; }
        ...
        .log-empty { display: flex; align-items: center; justify-content: center; height: 100%; color: #333; font-size: 11px; }
      ...
      <div className="container">
        ...
        <div className="grid-top">
          <div className="card">
            ...
            <div className="strategy-info" style={{ "--sc": activeStrategy.color } as React.CSSProperties}>
          ...
          <div className="card">
            ...
            <div className="gauges-row">
              <RadialGauge value={fw ? fw.count : 0}          max={10} color="#00ff88" label="Fixed Window"   />
              <RadialGauge value={sw ? sw.count : 0}          max={10} color="#00aaff" label="Sliding Window" />
              <RadialGauge value={tb ? Math.round(10 - tb.tokens) : 0} max={10} color="#ff6b35" label="Tokens Used"   />
            </div>
          </div>
        </div>
        ...
        <div className="log-panel">
          <div className="log-header">
            <div className="log-title">
              {logs.length > 0 && <span className="pulse" style={{ color: "#00ff88", marginRight: 6 }}>●</span>}
              Request Log
            </div>
